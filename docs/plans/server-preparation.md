...
| --- | --- | --- | --- |
| T01 | Workers上のRust・WebAuthn互換性検証 / Rust and WebAuthn compatibility spike | — | 部分完了・ブロック中: `workers-rs` 0.8.5 はWasmビルド・バンドル・ローカルHTTP応答まで成功。`webauthn-rs` 0.5.5 はOpenSSL依存とWasm向け乱数設定によりビルド不能、`passkey-auth` 0.1.3 は登録チャレンジ生成時の `std::time::SystemTime` パニックにより不採用。Workers互換のWebAuthn検証ライブラリを選定し、登録・認証応答の実検証を行うまでT01を完了にしない。 / Partially complete and blocked: `workers-rs` 0.8.5 built, bundled and served local HTTP. Reject `webauthn-rs` 0.5.5 because OpenSSL and Wasm randomness requirements prevent the build, and reject `passkey-auth` 0.1.3 because its registration challenge panics at `std::time::SystemTime`. Do not complete T01 until a Workers-compatible verification library is selected and real registration/authentication responses are validated. |
